...
class Agent(object):
	...
	def __init__(self,env,gamma=0.98,alpha=0.01,memory_size=5000,minibatch_size=32,update_freq=10):
	...
	def get_minibatch(self):
		...
		if len(self.memory) < self.minibatch_size:
			...
			return random.sample(list(self.memory),self.minibatch_size)
	...
	def target_qvalues(self,qvalues,actions,rewards,state_next):
		...
		q_statenext=self.target_network.predict(state_next)
	...
	def update_network(self,minibatch):
		...
		self.network.fit(state_now,target_qvalues,epochs=1)
	...
	def train(self,num_episodes=5000):
		...
		for i in range(num_episodes):
			...
			while j<1000:
				...
				qvalues=self.network.predict(state_now.reshape(1,self.state_size))
